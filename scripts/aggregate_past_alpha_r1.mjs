...
const INPUT_DIR=path.resolve(process.env.PA_INPUT_DIR??"runtime/past-alpha-r1-input");
const OUTPUT_DIR=path.resolve(process.env.PA_SUMMARY_DIR??"runtime/past-alpha-r1-summary");
...
let verdict="PAST_ALPHA_REJECT_R1";if(pass)verdict="PAST_ALPHA_SUPPORTED_R1";else if(allSeg&&b.cumulativeReturn>0)verdict="PAST_ALPHA_INTERESTING_NOT_PROVEN";
...
const summary={version:"PAST_ALPHA_R1",generatedAt:new Date().toISOString(),symbolsLoaded:bySymbol.size,verdict,base:compact(base),doubleCosts:compact(stress)};
await fs.writeFile(path.join(OUTPUT_DIR,"summary.json"),JSON.stringify(summary,null,2));await fs.writeFile(path.join(OUTPUT_DIR,"base-daily.json"),JSON.stringify(base.rows));await fs.writeFile(path.join(OUTPUT_DIR,"formations.json"),JSON.stringify(base.formationRows,null,2));console.log(`PA_SUMMARY=${JSON.stringify(summary)}`);