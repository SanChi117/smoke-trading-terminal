...
const SYMBOL=String(process.env.ILQ_SYMBOL??"BTCUSDT").trim().toUpperCase();
const OUTPUT_DIR=path.resolve(process.env.ILQ_OUTPUT_DIR??"runtime/illiquidity-premium-r1");
...
function mean(vals){const a=vals.filter(Number.isFinite);return a.length?a.reduce((s,v)=>s+v,0)/a.length:null}
...
if(candles.length<366){await fs.writeFile(path.join(OUTPUT_DIR,`${SYMBOL}.json`),JSON.stringify({version:"ILLIQUIDITY_PREMIUM_R1",symbol:SYMBOL,status:"INSUFFICIENT_DATA",records:[],fundingRates}));process.exit(0)}
const listingTime=candles[0].time,records=[];
for(let i=60;i<candles.length-1;i++){const c=candles[i],next=candles[i+1];if(c.time<REPORT_START||c.time>REPORT_END)continue;const hist=candles.slice(i-60,i+1);if(hist.length!==61||hist.at(-1).time-hist[0].time!==60*DAY)continue;const am=[];for(let j=1;j<hist.length;j++){const ret=hist[j].close/hist[j-1].close-1,qv=hist[j].quoteVolume;if(Number.isFinite(ret)&&Number.isFinite(qv)&&qv>0)am.push(Math.abs(ret)/qv)}if(am.length!==60)continue;const medQ=median(candles.slice(Math.max(0,i-29),i+1).map(x=>x.quoteVolume));records.push({symbol:SYMBOL,date:dateLabel(c.time),time:c.time,ageDays:round((c.time-listingTime)/DAY,3),close:round(c.close,10),amihud60:round(mean(am),18),medianQuoteVolume30:round(medQ,2),nextDate:dateLabel(next.time),nextTime:next.time,nextReturn:round(next.close/c.close-1,10)})}
const report={version:"ILLIQUIDITY_PREMIUM_R1",symbol:SYMBOL,status:"OK",generatedAt:new Date().toISOString(),listingDate:dateLabel(listingTime),records,fundingRates};
await fs.writeFile(path.join(OUTPUT_DIR,`${SYMBOL}.json`),JSON.stringify(report));
console.log(`ILQ_SYMBOL=${JSON.stringify({symbol:SYMBOL,status:"OK",records:records.length,fundingRates:fundingRates.length,listingDate:report.listingDate})}`);