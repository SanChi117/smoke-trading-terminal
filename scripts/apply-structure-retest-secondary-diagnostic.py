...
def main() -> None:
    ...
    old2 = '''  return {\n    ...base,\n    targetZone,'''
    new2 = '''  const selectorMeta = firstBarrier && targetZone && selectedRank > 1\n    ? `STRUCTURE_RETEST_SELECTOR|rank=${selectedRank}|firstId=${firstBarrier.id}|firstTf=${firstBarrier.timeframe}|firstSource=${firstBarrier.source}|firstTouches=${firstBarrier.touches}|firstScore=${firstBarrier.score}|firstLow=${firstBarrier.low}|firstHigh=${firstBarrier.high}`\n    : null;\n\n  return {\n    ...base,\n    selectorMeta,\n    targetZone,'''
