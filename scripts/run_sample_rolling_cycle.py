#!/usr/bin/env python3
"""Run rolling-symbol-strength cycle on generated sample trades."""
            ...
            for top_n in [5, 8, 12, 20]:
                for lev in [10, 15, 20, 25]:
                    result = run_rolling(
                        ...
                        CapitalConfig(initial_cash=500, risk_pct=0.02, leverage=lev, max_positions=2),
                        CostConfig(fee_rate=0.0010, slippage_rate=0.0002),
                    )
                    rows.append({
                        "setup": result.setup,
                        "lev": lev,
