...
const SYMBOL = String(process.env.DLO_SYMBOL ?? "BCHUSDT").trim().toUpperCase();
const OUTPUT_DIR = path.resolve(process.env.DLO_OUTPUT_DIR ?? "runtime/donchian-long-only-crossasset-oos-r1");
...
async function readZip(url, cacheKey) {
  ...
  const file = path.join(os.tmpdir(), `donchian-long-oos-${crypto.randomUUID()}.zip`);
  try {
...
function adverseEntryFill(open) {
  return open * (1 + SLIPPAGE);
}
function adverseExitFill(raw) {
...
if (candles.length < 1000 || candles.at(-1)?.time < Date.parse("2026-07-01T00:00:00.000Z")) {
  const insufficient = {
    version: "DONCHIAN_LONG_ONLY_CROSSASSET_OOS_R1",
    symbol: SYMBOL,
  ...
  console.log(`DONCHIAN_LONG_ONLY_CROSSASSET=${JSON.stringify({ symbol: SYMBOL, status: insufficient.status, candleCount: candles.length })}`);
  process.exit(0);
}
...
for (let i = ENTRY_LOOKBACK; i < candles.length; i += 1) {
  ...
  if (position && i >= EXIT_LOOKBACK) {
    ...
    if (candle.close < exitLow && i + 1 < candles.length) pendingChannelExit = true;
  }
  ...
  if (!position && !pendingEntry && candle.time + 4 * HOUR >= ENTRY_START && candle.time + 4 * HOUR <= ENTRY_END && i + 1 < candles.length) {
    ...
    if (Number.isFinite(currentAtr) && currentAtr > 0) {
      ...
      if (candle.close > entryHigh) {
        pendingEntry = { signalTime: candle.time + 4 * HOUR, atr: currentAtr };
        entrySignals += 1;
      }
    }
  }
...
const splitCounts = { EARLY: 0, Y2025: 0, Y2026_OOS: 0 };
for (const trade of trades) splitCounts[trade.split] = (splitCounts[trade.split] ?? 0) + 1;
const report = {
  version: "DONCHIAN_LONG_ONLY_CROSSASSET_OOS_R1",
  symbol: SYMBOL,
  status: "OK",
  ...
  candleCount: candles.length,
  firstCandle: iso(candles[0]?.time),
  lastCandle: iso(candles.at(-1)?.time),
  entrySignals,
  splitCounts,
  records: trades,
};
...
console.log(`DONCHIAN_LONG_ONLY_CROSSASSET=${JSON.stringify({ symbol: SYMBOL, status: "OK", trades: trades.length, splitCounts, entrySignals })}`);
