...
const INPUT_DIR=path.resolve(process.env.ILQ_INPUT_DIR??"runtime/illiquidity-premium-r1-input");
const OUTPUT_DIR=path.resolve(process.env.ILQ_SUMMARY_DIR??"runtime/illiquidity-premium-r1-summary");
...
let verdict="ILLIQUIDITY_PREMIUM_REJECT_R1";if(pass)verdict="ILLIQUIDITY_PREMIUM_SUPPORTED_R1";else if(allSeg&&b.cumulativeReturn>0)verdict="ILLIQUIDITY_PREMIUM_INTERESTING_NOT_PROVEN";
...
const summary={version:"ILLIQUIDITY_PREMIUM_R1",generatedAt:new Date().toISOString(),symbolsLoaded:bySymbol.size,verdict,base:compact(base),doubleCosts:compact(stress)};
await fs.writeFile(path.join(OUTPUT_DIR,"summary.json"),JSON.stringify(summary,null,2));await fs.writeFile(path.join(OUTPUT_DIR,"base-daily.json"),JSON.stringify(base.rows));await fs.writeFile(path.join(OUTPUT_DIR,"formations.json"),JSON.stringify(base.formationRows,null,2));console.log(`ILQ_SUMMARY=${JSON.stringify(summary)}`);