...
const [databasePath, afterId, kind] = process.argv.slice(2);
if (!databasePath || !existsSync(databasePath) || (kind && kind !== 'guardian')) throw new Error('Usage: node --experimental-strip-types scripts/reconcile-execution.mjs EXISTING_LEDGER.sqlite [cursor] [guardian]');
const store = kind === 'guardian' ? new GuardianStore(databasePath) : new ExecutionStore(databasePath);
try {
