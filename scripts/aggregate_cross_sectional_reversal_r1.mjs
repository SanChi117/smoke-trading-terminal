...
function averageSleeves(sleeves){
  const out=new Map(); if(!sleeves.length)return out;
  for(const sleeve of sleeves) for(const [s,w] of sleeve.weights) addWeight(out,s,w/sleeves.length);
  return out;
}
...
function runProfile(profile,stress){
  let sleeves=[]; let prevWeights=new Map(); const rows=[]; const formations=[]; const contributions=new Map();
  for(const time of allTimes){
    ...
    for(const [symbol,w] of weights){
      ...
      const c=w*r.nextReturn; grossReturn+=c;
      const key=`${symbol}|${w>=0?"long":"short"}`; contributions.set(key,(contributions.get(key)||0)+c);
    }
    const ex=exposure(weights);
    const cost=turn*COST_RATE;
    const funding=stress?FUNDING_STRESS_DAILY*ex.gross:0;
    const netReturn=grossReturn-cost-funding;
    rows.push({date:dateLabel(time),time,split:splitOf(time),profile,stress:stress?"FUNDING_STRESS":"BASE_COSTS",grossReturn,turnover:turn,cost,funding,netReturn,gross:ex.gross,net:ex.net,longExposure:ex.long,shortExposure:ex.short,activeSleeves:sleeves.length});
    prevWeights=weights;
  }
  const overall=metrics(rows);
  const splits={}; for(const s of ["EARLY","Y2025","Y2026_KNOWN"])splits[s]=metrics(rows.filter(r=>r.split===s));
  return {profile,stress:stress?"FUNDING_STRESS":"BASE_COSTS",overall,splits,formations,rows,contributions:[...contributions.entries()].map(([key,value])=>{const [symbol,side]=key.split("|");return {symbol,side,contribution:value};}).sort((a,b)=>b.contribution-a.contribution)};
}

const runs=[];
for(const p of ["BASE_LMW_8W","HIGHVOL_LMW_8W"]){runs.push(runProfile(p,false));runs.push(runProfile(p,true));}
function gate(run){
  const b=run.overall,s=run.splits;
  const medEligible=median(run.formations.map(x=>x.eligibleCount))||0;
  return b.cumulativeReturn>0&&b.sharpe>=0.75&&b.maxDrawdown>=-0.25&&s.EARLY.cumulativeReturn>0&&s.EARLY.sharpe>0&&s.Y2025.cumulativeReturn>=0&&s.Y2025.sharpe>=0&&s.Y2026_KNOWN.cumulativeReturn>=0&&s.Y2026_KNOWN.sharpe>=0&&run.formations.length>=100&&medEligible>=15;
}
const baseRuns=runs.filter(r=>r.stress==="BASE_COSTS");
let supported=null;
for(const br of baseRuns){const sr=runs.find(r=>r.profile===br.profile&&r.stress==="FUNDING_STRESS");if(gate(br)&&sr.overall.cumulativeReturn>0&&sr.splits.Y2026_KNOWN.cumulativeReturn>=0){supported=br.profile;break;}}
let verdict="REVERSAL_REJECT_R1";
if(supported)verdict="REVERSAL_MECHANISM_SUPPORTED";
else if(baseRuns.some(r=>r.overall.cumulativeReturn>0&&r.overall.sharpe>0))verdict="REVERSAL_INTERESTING_NOT_PROVEN";
...
const summary={version:"CROSS_SECTIONAL_REVERSAL_R1",generatedAt:new Date().toISOString(),symbolsLoaded:bySymbol.size,verdict,supportedProfile:supported,runs:compact};
await fs.writeFile(path.join(OUTPUT_DIR,"summary.json"),JSON.stringify(summary,null,2));
for(const r of runs){await fs.writeFile(path.join(OUTPUT_DIR,`${r.profile}-${r.stress}-daily.json`),JSON.stringify(r.rows));await fs.writeFile(path.join(OUTPUT_DIR,`${r.profile}-${r.stress}-contributions.json`),JSON.stringify(r.contributions,null,2));}
