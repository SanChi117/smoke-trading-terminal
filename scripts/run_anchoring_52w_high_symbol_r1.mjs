...
const SYMBOL=String(process.env.A52_SYMBOL??"BTCUSDT").trim().toUpperCase();
const OUTPUT_DIR=path.resolve(process.env.A52_OUTPUT_DIR??"runtime/anchoring-52w-high-r1");
...
function parseKlineCsv(csv){const out=[];for(const line of csv.trim().split(/\r?\n/)){const c=line.split(",");if(!Number.isFinite(Number(c[0])))continue;const high=Number(c[2]),close=Number(c[4]),baseVol=Number(c[5]),qv=Number(c[7]);const r={time:normalizeTime(c[0]),high,close,quoteVolume:Number.isFinite(qv)?qv:baseVol*close};if([r.time,r.high,r.close,r.quoteVolume].every(Number.isFinite))out.push(r)}return out}
...
if(candles.length<366){await fs.writeFile(path.join(OUTPUT_DIR,`${SYMBOL}.json`),JSON.stringify({version:"ANCHORING_52W_HIGH_R1",symbol:SYMBOL,status:"INSUFFICIENT_DATA",records:[],fundingRates}));process.exit(0)}
const listingTime=candles[0].time,records=[];
for(let i=364;i<candles.length-1;i++){const c=candles[i],next=candles[i+1];if(c.time<REPORT_START||c.time>REPORT_END)continue;const hist=candles.slice(i-364,i+1);if(hist.length!==365||hist.at(-1).time-hist[0].time!==364*DAY)continue;const high52=Math.max(...hist.map(x=>x.high));const medQ=median(candles.slice(i-29,i+1).map(x=>x.quoteVolume));records.push({symbol:SYMBOL,date:dateLabel(c.time),time:c.time,ageDays:round((c.time-listingTime)/DAY,3),close:round(c.close,10),high52:round(high52,10),nearness52:round(c.close/high52,12),medianQuoteVolume30:round(medQ,2),nextDate:dateLabel(next.time),nextTime:next.time,nextReturn:round(next.close/c.close-1,10)})}
const report={version:"ANCHORING_52W_HIGH_R1",symbol:SYMBOL,status:"OK",generatedAt:new Date().toISOString(),listingDate:dateLabel(listingTime),records,fundingRates};
await fs.writeFile(path.join(OUTPUT_DIR,`${SYMBOL}.json`),JSON.stringify(report));
console.log(`A52_SYMBOL=${JSON.stringify({symbol:SYMBOL,status:"OK",records:records.length,fundingRates:fundingRates.length,listingDate:report.listingDate})}`);
