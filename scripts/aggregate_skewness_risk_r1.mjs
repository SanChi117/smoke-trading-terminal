...
const INPUT_DIR=path.resolve(process.env.SK_INPUT_DIR??"runtime/skewness-risk-r1-input");
const OUTPUT_DIR=path.resolve(process.env.SK_SUMMARY_DIR??"runtime/skewness-risk-r1-summary");
...
let verdict="SKEWNESS_RISK_REJECT_R1";if(pass)verdict="SKEWNESS_RISK_SUPPORTED_R1";else if(allSeg&&b.cumulativeReturn>0)verdict="SKEWNESS_RISK_INTERESTING_NOT_PROVEN";
...
const summary={version:"SKEWNESS_RISK_R1",generatedAt:new Date().toISOString(),symbolsLoaded:bySymbol.size,verdict,base:compact(base),doubleCosts:compact(stress)};
await fs.writeFile(path.join(OUTPUT_DIR,"summary.json"),JSON.stringify(summary,null,2));await fs.writeFile(path.join(OUTPUT_DIR,"base-daily.json"),JSON.stringify(base.rows));await fs.writeFile(path.join(OUTPUT_DIR,"formations.json"),JSON.stringify(base.formationRows,null,2));console.log(`SK_SUMMARY=${JSON.stringify(summary)}`);
