...
const out=process.argv[3]??"qfvg-fs15-production-equivalence-summary.json";
const rows=[];
...
const same=(left,right,tolerance=0.0001)=>Math.abs(left-right)<=tolerance;
const expected={
  baseline:{trades:37,netR:4.8885},
  candidate:{trades:41,netR:4.9267},
  delta:{trades:4,netR:0.0382},
  directExtras:{trades:4,netR:0.0382},
  windows:{
    "oos-2024-12-31":{trades:1,netR:0.2161},
    "oos-2025-03-31":{trades:1,netR:-1.0334},
    "oos-2026-01-31":{trades:2,netR:0.8555}
  }
};
const equivalenceChecks={
  exactBaseline:baseline.trades===expected.baseline.trades&&same(baseline.netR,expected.baseline.netR),
  exactCandidate:candidate.trades===expected.candidate.trades&&same(candidate.netR,expected.candidate.netR),
  exactDelta:candidate.trades-baseline.trades===expected.delta.trades&&same(candidate.netR-baseline.netR,expected.delta.netR),
  exactDirectExtras:directExtras.trades===expected.directExtras.trades&&same(directExtras.netR,expected.directExtras.netR),
  exactPerWindow:Object.entries(expected.windows).every(([window,value])=>(
    perWindow[window]?.delta.trades===value.trades&&same(perWindow[window]?.delta.netR,value.netR)
  ))
};
const checks={
...
const pass=Object.values(checks).every(Boolean)&&Object.values(equivalenceChecks).every(Boolean);
const report={
  version:"SMOKE_V5_QFVG_FS15_PRODUCTION_EQUIVALENCE_V1",
  frozenProductionBase:"56ae74d6fb6dcefc07a2c3fe14664a9a4c21f182",
  candidateSource:"production candidate PR #57 / 2bb92db456e8f253cbac007ce5b16748ea982ad8",
  definition:"Behavior-equivalence replay of frozen main against the maintainable production implementation of QFVG_FS15. The result must reproduce the untouched OOS metrics from PR #56 exactly; no retuning is permitted.",
  ...
  invariantFailures,
  expected,
  checks:{...checks,...equivalenceChecks},
  verdict:pass?"PRODUCTION_EQUIVALENCE_PASS":"PRODUCTION_EQUIVALENCE_FAIL"
};
await writeFile(out,JSON.stringify(report,null,2));
console.log(JSON.stringify({verdict:report.verdict,baseline,candidate,deltaVsC:report.deltaVsC,directExtras:report.directExtras,checks:report.checks}));
