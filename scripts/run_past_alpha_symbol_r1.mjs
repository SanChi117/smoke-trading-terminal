...
const SYMBOL=String(process.env.PA_SYMBOL??"ETHUSDT").trim().toUpperCase();
const OUTPUT_DIR=path.resolve(process.env.PA_OUTPUT_DIR??"runtime/past-alpha-r1");
...
function olsAlpha(y,x){if(y.length!==x.length||y.length<2)return null;const mx=x.reduce((s,v)=>s+v,0)/x.length,my=y.reduce((s,v)=>s+v,0)/y.length;let cov=0,vx=0;for(let i=0;i<x.length;i++){cov+=(x[i]-mx)*(y[i]-my);vx+=(x[i]-mx)**2}const beta=vx>0?cov/vx:0;return my-beta*mx}
...
async function readZipRaw(url,key){await fs.mkdir(CACHE_DIR,{recursive:true});const cached=path.join(CACHE_DIR,`${key}.zip`);let bytes=null;try{bytes=await fs.readFile(cached)}catch{}if(!bytes){for(let a=0;a<5;a++){const res=await fetch(url,{cache:"no-store"});if(res.status===404)return null;if(res.ok){bytes=Buffer.from(await res.arrayBuffer());await fs.writeFile(cached,bytes);break}if(res.status!==429&&res.status<500)throw new Error(`Binance Vision ${res.status}: ${url}`);await sleep(700*(a+1))}}if(!bytes)throw new Error(`Binance Vision retry limit: ${url}`);const tmp=path.join(os.tmpdir(),`pa-${crypto.randomUUID()}.zip`);try{await fs.writeFile(tmp,bytes);const r=spawnSync("unzip",["-p",tmp],{encoding:"utf8",maxBuffer:128*1024*1024});if(r.status!==0)throw new Error(`unzip failed: ${r.stderr}`);return r.stdout}finally{await fs.rm(tmp,{force:true})}}
async function readKlines(url,key){const raw=await readZipRaw(url,key);return raw==null?null:parseKlineCsv(raw)}
async function klineRange(symbol,start,end){const rows=[],finalMonth=monthStart(end);for(let cur=monthStart(start);cur<=finalMonth;cur=nextMonth(cur)){const ml=monthLabel(cur),key=`pa-${symbol}-1d-${ml}`;const monthly=await readKlines(`${BASE}/monthly/klines/${symbol}/1d/${symbol}-1d-${ml}.zip`,key);if(monthly){rows.push(...monthly);continue}if(cur!==finalMonth)continue;const last=Math.min(end,nextMonth(cur)-1);for(let d=cur;d<=last;d+=DAY){const dl=dayLabel(d),dk=`pa-${symbol}-1d-${dl}`;const daily=await readKlines(`${BASE}/daily/klines/${symbol}/1d/${symbol}-1d-${dl}.zip`,dk);if(daily)rows.push(...daily)}}const m=new Map();for(const r of rows)if(r.time>=start&&r.time<=end)m.set(r.time,r);return[...m.values()].sort((a,b)=>a.time-b.time)}
async function fundingRange(symbol,start,end){const out=[];for(let cur=monthStart(start);cur<=monthStart(end);cur=nextMonth(cur)){const ml=monthLabel(cur),key=`pa-funding-${symbol}-${ml}`;const raw=await readZipRaw(`${BASE}/monthly/fundingRate/${symbol}/${symbol}-fundingRate-${ml}.zip`,key);if(raw==null)continue;for(const x of parseFundingCsv(raw))if(x.fundingTime>=start&&x.fundingTime<=end)out.push(x)}const m=new Map();for(const x of out)m.set(x.fundingTime,x);return[...m.values()].sort((a,b)=>a.fundingTime-b.fundingTime)}

await fs.mkdir(OUTPUT_DIR,{recursive:true});
const[candles,btc,fundingRates]=await Promise.all([klineRange(SYMBOL,LOAD_START,LOAD_END),klineRange("BTCUSDT",LOAD_START,LOAD_END),fundingRange(SYMBOL,FUNDING_START,FUNDING_END)]);
if(candles.length<366||btc.length<366){await fs.writeFile(path.join(OUTPUT_DIR,`${SYMBOL}.json`),JSON.stringify({version:"PAST_ALPHA_R1",symbol:SYMBOL,status:"INSUFFICIENT_DATA",records:[],fundingRates}));process.exit(0)}
const btcMap=new Map(btc.map(x=>[x.time,x]));
const listingTime=candles[0].time,records=[];
for(let i=60;i<candles.length-1;i++){const c=candles[i],next=candles[i+1];if(c.time<REPORT_START||c.time>REPORT_END)continue;const hist=candles.slice(i-60,i+1);if(hist.length!==61||hist.at(-1).time-hist[0].time!==60*DAY)continue;const ar=[],br=[];let ok=true;for(let j=1;j<hist.length;j++){const b0=btcMap.get(hist[j-1].time),b1=btcMap.get(hist[j].time);if(!b0||!b1){ok=false;break}ar.push(Math.log(hist[j].close/hist[j-1].close));br.push(Math.log(b1.close/b0.close))}if(!ok)continue;const alpha=olsAlpha(ar,br);const medQ=median(candles.slice(Math.max(0,i-29),i+1).map(x=>x.quoteVolume));records.push({symbol:SYMBOL,date:dateLabel(c.time),time:c.time,ageDays:round((c.time-listingTime)/DAY,3),close:round(c.close,10),alpha60:round(alpha,12),medianQuoteVolume30:round(medQ,2),nextDate:dateLabel(next.time),nextTime:next.time,nextReturn:round(next.close/c.close-1,10)})}
const report={version:"PAST_ALPHA_R1",symbol:SYMBOL,status:"OK",generatedAt:new Date().toISOString(),listingDate:dateLabel(listingTime),records,fundingRates};
await fs.writeFile(path.join(OUTPUT_DIR,`${SYMBOL}.json`),JSON.stringify(report));
console.log(`PA_SYMBOL=${JSON.stringify({symbol:SYMBOL,status:"OK",records:records.length,fundingRates:fundingRates.length,listingDate:report.listingDate})}`);