...
const INPUT_DIR=path.resolve(process.env.A52_INPUT_DIR??"runtime/anchoring-52w-high-r1-input");
const OUTPUT_DIR=path.resolve(process.env.A52_SUMMARY_DIR??"runtime/anchoring-52w-high-r1-summary");
...
const summary={version:"ANCHORING_52W_HIGH_R1",generatedAt:new Date().toISOString(),symbolsLoaded:bySymbol.size,verdict,base:compact(base),doubleCosts:compact(stress)};
await fs.writeFile(path.join(OUTPUT_DIR,"summary.json"),JSON.stringify(summary,null,2));await fs.writeFile(path.join(OUTPUT_DIR,"base-daily.json"),JSON.stringify(base.rows));await fs.writeFile(path.join(OUTPUT_DIR,"formations.json"),JSON.stringify(base.formationRows,null,2));console.log(`A52_SUMMARY=${JSON.stringify(summary)}`);
