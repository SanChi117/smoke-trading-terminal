...
function deriveRange(weekly: Candle[], daily: Candle[], price: number): MtfLevelAnalysis["range"] {
  ...
  const pivots = findPivots(source, source === daily ? 3 : 2, source === daily ? 3 : 2);
...
function buildFourHourRoute(
  ...
  const inside = latest.low <= zone.high && latest.high >= zone.low;
  ...
  const movingAway = distances.length >= 3
    && distances.at(-1)! > distances.at(-2)! + atr4h * 0.2
    && distances.at(-2)! > distances.at(-3)!;
  const state: FourHourRoute["state"] = invalidated
    ? "invalidated"
    : inside
      ? "inside"
      : movingAway
...
function analyzeReaction(
  ...
  const zoneWidth = Math.max(zone.high - zone.low, atrSeries.at(-1)! * 0.2);
  const touchLocalIndex = recent.findIndex((candle) => candle.low <= zone.high && candle.high >= zone.low);
  if (touchLocalIndex < 0) return emptyReaction(`5m: цена ещё не вошла в ${zone.label}`, side);
  const touchIndex = recentStart + touchLocalIndex;
  const candidates: Reaction[] = [];

  // Model A: liquidity sweep or deep test with a reclaim into the useful half of the zone.
  for (let index = touchIndex; index < candles.length; index += 1) {
    ...
    const score = Math.round(Math.min(100, 62 + Math.min(14, ratio * 7) + Math.abs(candle.close - candle.open) / Math.max(atrSeries[index], 1e-9) * 10));
    candidates.push({
      confirmed: true,
      side,
      type: "sweep_reclaim",
  ...
  // Model B: touch -> BOS/CHoCH with displacement -> actual retest of the broken micro level.
  ...
  for (const event of events) {
    ...
    if (eventIndex < touchIndex) continue;
    ...
    const retest = candles.slice(eventIndex + 1, eventIndex + 9).find((candle) => (
      side === "long"
  ...
  // Model C: direct displacement out of the zone. It is an alternative, not an additional mandatory gate.
  for (let index = touchIndex; index < candles.length; index += 1) {
    const candle = candles[index];
    const atr = atrSeries[index] || zoneWidth;
    const body = Math.abs(candle.close - candle.open);
    const startedInZone = candle.low <= zone.high && candle.high >= zone.low;
  ...
  const best = candidates
    .filter((reaction) => reaction.time !== null && reaction.time >= candles[touchIndex].time)
    .sort((a, b) => b.score - a.score || (b.time ?? 0) - (a.time ?? 0))[0];
  return best ?? emptyReaction("5m: касание есть, полноценной реакции ещё нет", side);
}
...
function confirm15m(
  ...
  const deadline = reaction.time + 2 * 60 * 60_000;
  ...
  for (const candle of after) {
    ...
    const zoneHeld = reaction.side === "long"
      ...
      : candle.close < zone.midpoint;
    if (directionOk && triggerOk && zoneHeld && body >= atr * 0.2) {
      return {
        confirmed: true,
        entry: candle.close,
        detail: `15m: реакция удержана, закрытие за 5m trigger (${reaction.type})`,
        time: candle.time,
      };
    }
  }
...
export function analyzeLevelFlow(
  ...
  if (route4h.state === "invalidated") blockers.push("4H закрылся сквозь выбранный уровень");
  if (route4h.state === "moving_away") blockers.push("4H движется от выбранной зоны, мониторинг входа преждевременен");
  if (activeZone && route4h.distanceAtr !== null && route4h.distanceAtr > 1.1) {
    blockers.push(`Цена ещё далеко от зоны: ${route4h.distanceAtr.toFixed(2)} ATR(4H)`);
  }
  if (!reaction.confirmed) blockers.push("Нет одной из допустимых полноценных моделей реакции 5m");
  if (!confirmation.confirmed) blockers.push("Нет подтверждения реакции закрытой свечой 15m");
  ...
  confidence += trendStrength === "strong" ? 12 : trendStrength === "normal" ? 8 : 3;
  confidence += activeZone ? Math.min(22, activeZone.score * 0.27) : 0;
  confidence += route4h.state === "inside" ? 12 : route4h.state === "approaching" ? 8 : 0;
    ...
    trace: [
      {
        ...
        detail: `1W ${weeklyBias}; 1D ${dailyBias}; рабочее направление ${bias}; сила ${trendStrength}; положение ${range?.position ?? "n/a"}`,
      },
      {
        id: "level",
      ...
      },
      {
        ...
        state: route4h.state === "inside" || route4h.state === "approaching" ? "pass" : activeZone ? "fail" : "pending",
